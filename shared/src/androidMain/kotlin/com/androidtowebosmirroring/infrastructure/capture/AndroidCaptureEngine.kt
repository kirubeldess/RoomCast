...
import com.androidtowebosmirroring.infrastructure.network.Dlna
import com.androidtowebosmirroring.infrastructure.network.LiveServer
...
/**
 * Android capture adapter. Owns codecs, projection, and the render/control workers.
 * Audio/video timestamps and drain scheduling intentionally remain together.
 */
class AndroidCaptureEngine(
    private val context: Context,
    private val consent: Intent,
    ...
    override val stopRequested: Boolean get() = stopped.get()
    @Suppress("DEPRECATION", "MissingPermission")
    override fun run(request: MirrorRequest) {
        val tv = request.receiver
        receiver = tv
        val withAudio = request.audio
        check(!withAudio || context.checkSelfPermission(Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED) {
            "Allow audio permission or turn Internal audio off"
        }
        projection = context.getSystemService(MediaProjectionManager::class.java).getMediaProjection(Activity.RESULT_OK, consent)
        val projection = projection ?: error("Screen capture approval expired")
        projection.registerCallback(callback, Handler(Looper.getMainLooper()))
        val metrics = android.util.DisplayMetrics()
        val physicalDisplay = context.getSystemService(DisplayManager::class.java).getDisplay(android.view.Display.DEFAULT_DISPLAY)
        physicalDisplay.getRealMetrics(metrics)
        val shortSide = request.quality.coerceIn(480, 1080)
        ...
        renderer = ScreenRenderer(request.videoMode)
        renderWorker.submit {
        ...
        server = LiveServer(tv) { sessions.update { if (it.active) it.copy(message = "${tv.name} requested the stream. Waiting for playback…") else it } }
        ...
        var longestGapMs = 0L
        try {
            sessions.update { it.copy(message = "Asking ${tv.name} to open the live stream…") }
            while (!stopped.get()) {
                ...
                if (nowNs - statsStartNs >= 2_000_000_000L) {
                    ...
                    sessions.update { if (it.active) it.copy(diagnostics = details) else it }
                ...
                if (!controlChecked && playback.isDone) {
                    ...
                    try {
                        playback.get()
                        sessions.update { if (it.active) it.copy(message = "TV accepted playback. Sending screen and sound to ${tv.name}.") else it }
                        ...
                        if (Dlna.canContinueAfterTimeout(reason, stream.hasWrittenMedia, System.nanoTime() - stream.lastReadNanos)) {
                            ...
                            sessions.update { if (it.active) it.copy(message = "TV is receiving the stream but did not acknowledge Play. Continuing while data flows.") else it }
                        } else throw e
                    }
                }
                audio?.let { codec ->
    ...
    override fun requestStop() { stopped.set(true) }
    override fun close() {
        ...
        sessions.update { it.copy(message = "Screen and audio capture are off. Closing the TV connection…") }
        control.shutdownNow()
